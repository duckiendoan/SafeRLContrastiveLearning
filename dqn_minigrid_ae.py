...
"""
    ...
    safe_q_network = PixelQNetwork(envs).to(device)
    safe_q_network.load_state_dict(torch.load('safeqnetwork.cleanrl_model', map_location=device))
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if args.ae_path is None and global_step % args.vae_training_frequency == 0:
                ...
                with torch.no_grad():
                    safe_obs_q = safe_q_network(data.observations)
                    mean_q = safe_obs_q.mean(dim=1, keepdim=True)
                    real_q = safe_obs_q.gather(1, data.actions)
                    target_q = real_q - mean_q + 0.05

                latent_dist = torch.cdist(latent, latent)
                dist_weights = target_q @ target_q.T
                dist_weights.fill_diagonal_(0.0)
                latent_dist_loss = 0.5 * torch.mean(latent_dist * torch.sign(dist_weights))
                loss = args.reconstruct_loss_coef * reconstruct_loss + args.vae_beta * latent_norm + latent_dist_loss
                ...
                if global_step % 100 == 0:
                    ...
                    writer.add_scalar("losses/positive_weight_mean", dist_weights[dist_weights > 0].mean().item(),
                                      global_step)
                    writer.add_scalar("losses/negative_weight_mean", dist_weights[dist_weights < 0].mean().item(),
                                      global_step)
