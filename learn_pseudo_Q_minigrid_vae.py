...
"""
    ...
    for global_step in range(args.total_timesteps):
        ...
        epsilon = linear_schedule(args.start_e, args.end_e, args.exploration_fraction * args.total_timesteps,
                                  global_step)
        obs_embedding, _, _ = encoder.sample(torch.Tensor(obs).to(device), deterministic=args.deterministic_latent)
        ...
        next_obs_embedding, _, _ = encoder.sample(torch.Tensor(next_obs).to(device), deterministic=args.deterministic_latent)
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.vae_training_frequency == 0:
                ...
                next_latent = encoder.sample(data.next_observations)[0]
                latent, mu, log_var = encoder.sample(data.observations)
                ...
                # aggregate
                loss = adjacent_loss + args.reconstruct_loss_coef * reconstruct_loss + args.vae_beta * kl_loss
                if global_step % 100 == 0:
    ...
    if args.save_model:
        ...
        ae_model_path = f"runs/{run_name}/{args.exp_name}_vae.pt"
        torch.save({
