...
from utils import TransposeImageWrapper, NearestHazardCostWrapper
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.cost_critic = layer_init(nn.Linear(512, 1), std=1)

    def get_value(self, x):
        return self.critic(self.network(x / 255.0))

    def get_cost_value(self, x):
        return self.cost_critic(self.network(x / 255.0))
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(hidden), self.cost_critic(hidden)
...
if __name__ == "__main__":
    ...
    # Optimize cost penalty parameter
    penalty_param = torch.tensor(args.langrangian_lambda, requires_grad=True).float()
    penalty_optimizer = optim.Adam([penalty_param], lr=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Calculate cost value function
            next_cvalue = agent.get_cost_value(next_obs).reshape(1, -1)
        ...
        cost_dev = average_ep_cost - args.cost_limit
        # update lambda
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_cadvantages = b_cost_advantages[mb_inds]
                if args.norm_adv:
                ...
                ppo_lg_objective = pg_loss - args.ent_coef * entropy_loss + penalty_item * cpg_loss
                ppo_lg_objective = ppo_lg_objective / (1 + penalty_item)
                ...
                if args.clip_vloss:
                    cv_loss_unclipped = (newcvalue - b_cost_returns[mb_inds]) ** 2
